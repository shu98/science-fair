...
rownames(clin11info)<-clin11info[,3]
clin11info<-clin11info[,c(5,9,15,18:20,39)]
...
dead.above.clin<-unique(dead.above.clin)

#manually remove NA
is.na(rownames(dead.below.clin))
is.na(rownames(dead.above.clin))

source("./Scripts/data_compilation_script_DNA methylation.R")

dead.below<-b11.data[,rownames(dead.below.clin)]
dead.above<-b11.data[,rownames(dead.above.clin)]
