...
genes.on.chip<-getSYMBOL(rownames(b9.data), "hgu133a2.db")
sets <- getGmt("./Downloaded Data/c2.cp.v4.0.symbols.gmt")
genes.by.pathway <- lapply(sets, geneIds)
names(genes.by.pathway) <- names(sets)

genes.of.interest<-read.table("./miRNA/miRNA_batch 9_tumor/mirtarbase_overexp_batch 9.txt",stringsAsFactors=F,header=T,sep="\t")
genes.of.interest<-genes.of.interest[,3]
...
# write.table(enriched.pathway,file="./Gene Expression/gene exp_batch 9_tumor/enriched pathways_underexp.txt",sep="\t",quote=F)
